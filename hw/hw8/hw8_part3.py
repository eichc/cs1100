...
Full version of the simulation. Print the initial state of the simulation.
Then, run until there are no bears left or no berries and active bears. Print
the simulation every 5 turns, and again at the end of the simulation.
"""
...
if __name__ == "__main__":    
    ...
    turn = 1
    bear_entered = False
    tourist_entered = False
    #run until there are no active and reserve bears, or until there are no
    #berries and active bears
    while len(active_bears) + len(reserve_bears) > 0 and \
    not (len(active_bears) == 0 and total_berries == 0):
        print("\nTurn:", turn)
        #grow the berries
        field.grow()
        field.spread()
        ...
        #check if a bear should be added
        if len(reserve_bears) > 0 and total_berries >= 500:
            new_bear = reserve_bears.pop(0)
            active_bears.append(new_bear)
            bear_entered = True
        else:
            bear_entered = False
        
        #check if a tourist should be added
        if len(reserve_tourists) > 0 and len(active_bears) > 0:
            new_tourist = reserve_tourists.pop(0)
            active_tourists.append(new_tourist)
            tourist_entered = True
        else:
            tourist_entered = False
        ...
        #print the bears and tourists that have left and entered
        for bear in bears_left:
            print("{} - Left the Field".format(bear))
        for tourist in tourists_left:
            print("{} - Left the Field".format(tourist))
        if bear_entered:
            print("{} - Entered the Field".format(new_bear))
        if tourist_entered:
            print("{} - Entered the Field".format(new_tourist))
        
        #print state of simulation every 5 turns
        if turn % 5 == 0:
            print("Field has {} berries.".format(total_berries))
            print(display)
            print("Active Bears:")
